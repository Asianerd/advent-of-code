...
raw_rules = []
for i in raw_data_string.strip().split("\n\n")[0].strip().split("\n"):
    raw_rules.append(i)

raw_rules.sort(key=lambda x:int(x.split(": ")[0]))
...
for x in raw_rules:
    ...
    # "a" / "b"
    _after = x.split(": ")[-1]
    if ("a" in _after) or ("b" in _after):
        int_rules.append(x[-2])
    else:
        batches = ([i.strip() for i in _after.split(" | ")]) if ("|" in x) else ([_after.strip()])
        int_rules.append([])
        for i in batches:
            int_rules[-1].append([int(n) for n in i.split(" ")])

# for index, x in enumerate(int_rules):
#     print(f'{index}. {x}')
...
def fetch(index):
    results = []
    if int_rules[index] in ['a', 'b']:
        return int_rules[index]
    for x in int_rules[index]:
        for i in x:
