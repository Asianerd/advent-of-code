...
class Bag:
    ...
    def __init__(self, text: str):
        self.type = text.strip().split(" bags contain ")[0]
        self.contains = {}

        self.text = text

    def find_container(self):
        if self.text.strip().split(" bags contain ")[1] != 'no other bags.':
            contains = self.text.strip().split(" bags contain ")[1][0:-1].strip().split(", ")
            for x in contains:
                _type = ' '.join(x.strip().split(" ")[1:-1])
                _amount = int(x.strip().split(" ")[0])
                self.contains[Bag.search(_type)] = _amount
        # print(f":{self.type}:")
        # print('\t' + '\n\t'.join([f'{x[0].type} : {x[1]}' for x in self.contains.items()]))

    def can_contain(self, _t):
        # print(f"{self.type} - {_t}")
        if _t in [i.type for i in self.contains.keys()]:
            # print("Found!!")
            return True
        for x in self.contains.keys():
            if x.can_contain(_t):
                return True

    @staticmethod
    def search(_t):
        return [x for x in Bag.bags if x.type == _t][0]


for x in raw_data_input.strip().split("\n"):
    Bag.bags.append(Bag(x))
