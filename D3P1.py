...
class Line:
    ...
    def __init__(self, instruction):
        ...
        for x in instruction.split(","):
            current_increment = Line.increment_type[x[0]]
            for r in range(0,int(x[1:])):
                current_pos[0] += current_increment[0]
                current_pos[1] += current_increment[1]
                self.points.append(list(current_pos))

    @staticmethod
    def display_grid(pointLists):
        amount = 15
        grid = []
...
intersections = []
for index, a in enumerate(lineA.points):
    if a in lineB.points:
        intersections.append(list(a))

# Line.display_grid([lineA.points, lineB.points])
# Line.display_grid([lineA.points])
# Line.display_grid([lineB.points])

print(len(intersections))

closest = 1000000000000000000000000000000000000000000000000000000
