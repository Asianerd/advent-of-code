...
class Line:
    ...
    def __init__(self, instruction):
        ...
        for x in instruction.strip().split(","):
            increment = increment_states[x[0]]
            for i in range(int(x[1:])):
                pos[0] += increment[0]
                pos[1] += increment[1]
                self.points.append(list(pos))
                ...
                # x check
                if pos[0] < Line.ranges[0][0]:
                    Line.ranges[0][0] = pos[0]
                    continue

                if pos[0] > Line.ranges[0][1]:
                    Line.ranges[0][1] = pos[0]
                    continue
                #

                # y check
                if pos[1] < Line.ranges[1][0]:
                    Line.ranges[1][0] = pos[1]
                    continue

                if pos[1] > Line.ranges[1][1]:
                    Line.ranges[1][1] = pos[1]
                    continue
                #
...
grid = []
for y in range(Line.ranges[1][0], Line.ranges[1][1] + 1):
    grid.append([])
    for x in range(Line.ranges[0][0], Line.ranges[0][1] + 1):
        grid[-1].append(0)
...
for list_index in range(2):
    for raw_point in [lineA.points, lineB.points][list_index]:
        point = [
            raw_point[0] - Line.ranges[0][0],
...
#         print(f'\33[92mH' if (
#             (x == center[0]) and (y == center[1])
#         ) else (('\33[93m' if i == 3 else '\33[0m') + ".##@"[i]), end='')
#     print()
...
for list_index in range(2):
    ...
    for raw_point in [lineA.points, lineB.points][list_index]:
        ...
        if grid[point[1]][point[0]] == 3:
            if not (str(point) in counts.keys()):
                counts[str(point)] = int(count)
            else:
                counts[str(point)] += int(count)
print("count assign done")
get_now()
