...
count = -1
for index, x in enumerate(rucksacks):
    count += 1
    if count >= 3:
        count = 0
        collection.append([])
    collection[-1].append(x)
...
for c in collection:
    commons = list(set(set(c[0]).intersection(c[1])).intersection(c[2]))[0]
    score += scores.index(commons) + 1
...
# for x in rucksacks:
#     items.append(
#         list(set(x[0]).intersection(x[1]))[0]
#     )
