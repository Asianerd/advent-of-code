...
valids = 0
for item in raw_data_input.strip().split("\n"):
    security = item.split(": ")[0]
    password = item.split(": ")[1]
    
    wanted_range = range(int(security.split(" ")[0].split("-")[0]), int(security.split(" ")[0].split("-")[1]) + 1)
    wanted_letter = security.split(" ")[1]

    validated = False
    if len([x for x in password if x == wanted_letter]) in wanted_range:
        valids += 1
        validated = True

    print(f"{security} : {password}\n"
          f"{wanted_letter} : {wanted_range} = {validated}")
